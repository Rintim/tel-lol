...
game.import("character", (lib, game, ui, get, ai, _status) => {
	...
	const TEL_LOL = {
		...
		character: {
			...
			lol_darius: ["male", "tel_lol_nuo", 4, ["tel_lol_chuwang", "tel_lol_qunlang"], ["ext:英雄联盟扩展/image/character/darius.png", "des:“历史只记得胜利者。顺应诺克萨斯，你将名垂千古。”</br>提到诺克萨斯力量的象征，帝国上下没有人能比德莱厄斯这名久经沙场的战士更加适合。他从无名小卒逐渐成长为诺克萨斯之手，劈开了无数敌人的身躯 —— 其中也不乏诺克萨斯自己人。他从不怀疑自己执行的公义，也从不会在举起战斧后迟疑。作为崔法利军团指挥官，德莱厄斯的任何对手都不用指望他手下留情。"], "darius", "From Rintim, Ansory Solution/Theopse"],
			tel_nlol_Nautilus: ["male", "tel_lol_gang", 8, ["tel_lol_fuchen"], ["ext:英雄联盟扩展/image/character/nautilus.png"], "nautilus"],
			"tel_tlol_TahmKench": ["male", "other", 8, ["tel_lol_tunna"], ["ext:英雄联盟扩展/image/character/tahm_kench.png"], "tahm_kench"],
			"tel_alol_Ahri": ["female", "tel_lol_ai", 3, ["tel_lol_monsflame", "tel_lol_gotSoul"], ["ext:英雄联盟扩展/image/character/ahri.png", "des:天生就与精神领域的魔法存在连接的阿狸，是一名狐狸模样的瓦斯塔亚，在世界上寻找着自己所属的位置。进入凡人社会以后，她成为了一名充满悔意和同情心的掠食者，她喜欢操纵猎物的情绪，然后再吸食他们的生命精魄——每吞噬一个灵魂，都伴着他们生前的记忆片段与领悟洞见。"], "ahri"]
		},
		characterIntro: {
		...
		skill: {
			...
			// Darius
			"tel_lol_chuwang": {
				audio: "ext:英雄联盟扩展/audio/skill:3",
				locked: true,
				forced: true,
			...
			// monsflame: 妖焰
			"tel_lol_monsflame": {
				audio: "ext:英雄联盟扩展/audio/skill:4",
				locked: true,
	...
	if (lib.device || lib.node) {
		for (const i in TEL_LOL.character) {
			if (!TEL_LOL.character[i][4].some(j => j.indexOf("ext:") != -1)) TEL_LOL.character[i][4].push(`ext:英雄联盟扩展/image/character/${i}.png`);
		}
	}
	else {
		for (const i in TEL_LOL.character) {
			if (!TEL_LOL.character[i][4].some(j => j.indexOf("ext:") != -1)) TEL_LOL.character[i][4].push(`db:extension-英雄联盟扩展:image/character/${i}.png`);
		}
	}
	return TEL_LOL;
});